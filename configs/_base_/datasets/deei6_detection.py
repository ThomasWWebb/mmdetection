...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/deei6_merged_raw_train.json',
        img_prefix=data_root + 'image/train/',
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/deei6_merged_raw_val.json',
        img_prefix=data_root + 'image/val/',
        pipeline=test_pipeline),
    test=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/deei6_merged_raw_test.json',
